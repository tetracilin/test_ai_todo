...
AGENT_ID="699a6738-f030-404c-8870-bd2b25625a09"
...
AGENT_FILE=/tmp/k19-agent-id.txt
...
for i in $(seq 1 60); do
  sleep 10
  RUN=$($DB "select id || '|' || status from heartbeat_runs where agent_id='$AGENT_ID' order by started_at desc nulls last limit 1")
  CNT=$(docker exec t3-prod-db-1 psql -U paperclip -d paperclip -tAc "select count(*) from heartbeat_runs where agent_id='$AGENT_ID'")
  echo "poll $i: runs=$CNT latest=${RUN:-none}"
  case "${RUN##*|}" in
    succeeded|completed|failed|interrupted) break ;;
  esac
done
echo "== final run state =="
docker exec t3-prod-db-1 psql -U paperclip -d paperclip -c "select id, status, started_at, completed_at, error_code from heartbeat_runs where agent_id='$AGENT_ID' order by started_at desc nulls last limit 3" 2>/dev/null || docker exec t3-prod-db-1 psql -U paperclip -d paperclip -c "select * from heartbeat_runs where agent_id='$AGENT_ID' limit 2"
