#!/usr/bin/env python3
"""K19: retry insert without explicit id (let default generate) and capture ids."""
import base64
import hashlib
import json
import os
...
sql = f"""
begin;
insert into company_secrets (company_id, name, provider, key)
values ('{comp_id}', 'hermes_gateway.apikey.k19-temp', 'local_encrypted', 'hermes_gateway.apikey.k19-temp')
returning id;

set constraint k19_dummy = 1;
commit;
"""
# simpler: two steps, non-transactional
sql1 = f"""insert into company_secrets (company_id, name, provider, key)
values ('{comp_id}', 'hermes_gateway.apikey.k19-temp', 'local_encrypted', 'hermes_gateway.apikey.k19-temp')
returning id;"""
open("/tmp/k19-s1.sql", "w").write(sql1)
subprocess.run(["docker", "cp", "/tmp/k19-s1.sql", "t3-prod-db-1:/tmp/k19-s1.sql"])
out1 = subprocess.run(
    ["docker", "exec", "t3-prod-db-1", "psql", "-U", "paperclip", "-d", "paperclip", "-tAc", "-f", "/tmp/k19-s1.sql"],
    capture_output=True, text=True,
)
print("insert secret:", out1.stdout.strip(), out1.stderr[:200])
