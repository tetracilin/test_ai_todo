...
export const PersonManagementView: React.FC = () => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (!isEditing || !formData.id) {
             alert('Please select a person to edit first.');
             return;
        }
        ...
        const personToSave: Person = {
            id: formData.id,
            name: formData.name,
            email: formData.email!, // Email is read-only
    ...
    return (
        <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6">
            <div className="bg-surface dark:bg-surface-dark p-4 rounded-lg border border-border-light dark:border-border-dark">
                <h3 className="text-lg font-semibold mb-4">{isEditing ? `Edit ${formData.name}` : 'Select a Person to Edit'}</h3>
                {isEditing ? (
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div>
                                <label htmlFor="email" className="block text-sm font-medium text-text-secondary dark:text-text-secondary-dark">Email (Read-only)</label>
                                <input type="email" name="email" id="email" value={formData.email || ''} readOnly className="mt-1 block w-full p-2 bg-gray-200 dark:bg-gray-900 rounded-md border-transparent focus:ring-0 cursor-not-allowed"/>
                            </div>
                            <div>
                ...
                ) : <p className="text-center text-text-secondary py-8">Select a person from the list below to edit their profile.</p>}
            </div>
             ...
             <div className="bg-surface dark:bg-surface-dark rounded-lg border border-border-light dark:border-border-dark overflow-hidden">
                <h3 className="text-lg font-semibold p-4 border-b border-border-light dark:border-border-dark">Registered Persons ({users.length})</h3>
