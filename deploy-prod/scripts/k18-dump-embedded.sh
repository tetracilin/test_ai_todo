...
# Containers run with --rm (auto-cleanup, no fixed names -> no conflicts).
set -eu
...
docker run --rm \
  ...
  > "$TEMP" || { printf 'pg_dump failed\n' >&2; exit 1; }

docker run --rm -i \
  --network host \
  postgres:18.1-alpine \
  pg_restore --list < "$TEMP" > /dev/null || { printf 'dump verification failed\n' >&2; exit 1; }
