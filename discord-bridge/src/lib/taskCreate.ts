...
export function validateTaskCreateInput(input: Pick<DiscordTaskCreateRequest, "title" | "description" | "priority">): string | null {
...
export async function createTaskFromDiscord(
  ...
  try {
    ...
    return `${action} **${result.issue.identifier}**: ${result.issue.title} — <${result.issue.url}>`;
  } catch (error) {
