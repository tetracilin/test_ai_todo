...
- [x] **Set `DISCORD_WEBHOOK_URL` as a repository secret** — **done 2026-09-17.** Confirmed via the Actions API: runs `35201279125` and later show `WEBHOOK: ***` (set) in the report step logs, versus `WEBHOOK:` (empty) in every run before it. (P1, human: ~10min / CC: 0 — needs repo settings access)
  - What: populate the secret every report step reads, so failures actually post.
  - Why (historical, true before 2026-09-17): all seven t3-nightly runs to that point produced zero alerts. The deploy job logged `DISCORD_WEBHOOK_URL not set; skipping`; the `slow-tests` report step exited silently on `[[ -n "$WEBHOOK" ]] || exit 0`. CLAUDE.md promises "a failed deploy or e2e is reported to Discord with a link to the run" — that didn't happen yet, which is why the other items here went unnoticed for five days.
  - Context: it must be a **repo**-level secret, not an environment secret. The `deploy` job declares `environment: staging`, but `slow-tests` (`t3-nightly.yml:199`, `runs-on: ubuntu-latest`) declares no environment at all, so an environment-scoped secret would have been invisible to it — and the slow-tests alert is the one that would have caught the test regressions. PLAN_CICD.md §2.3 already says repo secret. Channel 1534836487772704800.

- [x] **Create the nightly artifact-storage secrets on kmv8** — **done 2026-09-08** (`PLAN_CICD.md` §0). The `build-and-deploy-nightly` job has succeeded on every run since, most recently deploying `f6de6fba` on 2026-09-17 with a matching health check. (P1, human: ~15min if staging MinIO credentials exist, ~1h if a staging bucket + user must be minted / CC: 0 — agents must not touch kmv8)
  ...
  - Why (historical, true before 2026-09-08): PR #78 made both mandatory in `t3-nightly.yml`'s fail-early check and in `deploy/compose.yaml`, and merged without the host-side prerequisite its own commit message demanded. Staging had been frozen at `c3c03e81` (2026-09-03) as a result — resolved once the files above were created.
  - Context: staging must never carry production bucket credentials. `deploy/paperclip-config.json` currently sets `storage.provider = "local_disk"` with no `accessKeySecretRef`, so nothing reads the values yet — placeholders unblocked the deploy, but leave a trap for whoever first points staging at S3.

- [x] **Fix the two server-test regressions on develop** — **done, PR #103, 2026-09-17.** `slow-tests` is still routinely red, but now on one different timing-sensitive test per night out of ~4,800 (see `CICD/PLAN_AI_FACTORY.md` §0.2, phase 1.1), not on these two named regressions. (P2, human: ~1-2h / CC: ~20min)
...
- [x] **Land or close the seven stale Dependabot PRs** — **closed 2026-09-18** (`CICD/PLAN_AI_FACTORY.md` Phase 0.3), all seven still stale/unrebased. The monthly grouped Dependabot run will reopen current PRs against the current lockfile, which is the safer path this item itself recommends below. (P2, human: ~1h spread over days / CC: ~10min per PR)
