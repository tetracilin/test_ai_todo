...
export function breakdownSummarySentence(
  ...
  const parts: string[] = [
    `Purpose Robot will create one ${noun} per item in ${names.targetPipelineName} → ${names.entryStageName}`,
  ];
...
/**
 * The read-only "Purpose Robot handles this" mechanics bullets, composed from the
