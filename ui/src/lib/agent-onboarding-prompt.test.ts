...
describe("buildAgentOnboardingPrompt", () => {
  it("includes Hermes Gateway setup guidance for external agents", () => {
    ...
    expect(prompt).toContain("not the Purpose Robot API key");
    ...
    expect(prompt).toContain("Purpose Robot maps it to `/api`");
    ...
    expect(prompt).toContain("`hermes_local` runs Hermes on the Purpose Robot host");
    expect(prompt).toContain("Hermes-originated Purpose Robot API calls");
    expect(prompt).toContain("Do not rotate or invent a Purpose Robot key manually");
