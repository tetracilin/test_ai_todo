...
describe("MarkdownBody", () => {
  ...
  it("renders user, agent, project, skill, and routine mentions as chips", () => {
    const html = renderToStaticMarkup(
      <QueryClientProvider client={new QueryClient()}>
        <ThemeProvider>
          <MarkdownBody>
            {`[@Taylor](${buildUserMentionHref("user-123")}) [@CodexCoder](${buildAgentMentionHref("agent-123", "code")}) [@Purpose Robot App](${buildProjectMentionHref("project-456", "#336699")}) [/release-changelog](${buildSkillMentionHref("skill-789", "release-changelog")}) [/routine:Weekly review](${buildRoutineMentionHref("routine-123")})`}
          </MarkdownBody>
        </ThemeProvider>
      </QueryClientProvider>,
    );
  ...
  it("renders wiki links with a custom resolver when enabled", () => {
    const html = renderMarkdown(
      "See [[wiki/entities/paperclip|Purpose Robot]] and [[wiki/entities/dotta-b]].",
      [],
      {
    ...
    expect(html).toContain(">Purpose Robot</a>");
